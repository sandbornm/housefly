...
import { decodeEscape, encodeFlysim, FLYSIM_ACTIONS, FLYSIM_ENCODER, NEURAL_WINDOW_MS, unpowered, type FlysimNeuralFrame, type FlysimObservation } from './neural.ts';

export interface FlysimRuntime {
  advance(inputs: Float32Array, milliseconds: number): FlysimNeuralFrame | Promise<FlysimNeuralFrame>;
  reset(): void | Promise<unknown>;
  silence(value: boolean): void | Promise<unknown>;
  snapshot(): FlysimNeuralFrame | Promise<FlysimNeuralFrame>;
...
export class NeuralPilot {
  ...
  constructor(runtime: FlysimRuntime, model: { modelId: string; edgeCount: number }) {
    this.runtime = runtime; this.model = model;
    this.readout = new NeuralReadout({ actions: FLYSIM_ACTIONS, modelId: `${model.modelId}:${FLYSIM_ENCODER}`, seed: 8731 });
  }
  ...
  private async advance(inputs: Float32Array, milliseconds: number, reset = false): Promise<FlysimNeuralFrame> {
  ...
  async infer(observation: FlysimObservation): Promise<void> {
    ...
    await this.run(async revision => {
      ...
      this.stage(revision, () => {
        ...
        this.command = this.phase === 'inference' ? decodeEscape(frame, this.readout, observation)
  ...
  export(): ReturnType<NeuralReadout['export']> { return this.readout.export(); }
  ...
  async calibrateStep(): Promise<void> {
    ...
    await this.run(async revision => {
      ...
      const observation = calibrationObservation(this.samples), input = encodeFlysim(observation);
